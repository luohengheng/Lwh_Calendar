export default {
    ...
    sundayStart: false,
    // 判断传过来的两个值的的时间大小
    judgeTwoDate(date1, date2) {
        let gettype=Object.prototype.toString
        if(gettype.call(date1) == '[object String]' && gettype.call(date2) == '[object String]') {
            date1 = new Date().setFullYear(date1.split('/')[0], date1.split('/')[1] - 1, date1.split('/')[2])
            date2 = new Date().setFullYear(date2.split('/')[0], date2.split('/')[1] - 1, date2.split('/')[2])
            if(date1 <= date2) {
                return true
            }else {
                return false
